...
Metrics are protected under basic auth since prometheus scrappers can handle it by default. System metrics (cadvisor) have a special set of .htpasswd credentials.

To add an user and password to that basic auth execute:

```bash
# for catalyst metrics:
htpasswd -bc local/nginx/auth/.htpasswd-metrics [username] [password]
# for cadvisor container metrics:
htpasswd -bc local/nginx/auth/.htpasswd-cadvisor-metrics [username] [password]
```
